...
Public Sub RollReportingPacks_FromControl()
    ...
    Set ctlWs = EnsureSheetListReady()
    If ctlWs Is Nothing Then Exit Sub
...
Private Function EnsureSheetListReady() As Worksheet

    Dim ctlWs As Worksheet

    Set ctlWs = GetWorksheetIfExists("SheetList")

    If ctlWs Is Nothing Then
        Set ctlWs = CreateSheetListFromVisibleSheets()
        MsgBox "SheetList did not exist, so a new SheetList has been created from the Menu logic." & vbCrLf & vbCrLf & _
               "Visible sheet names have been listed in Column A." & vbCrLf & _
               "Please review and fill Columns B:E as needed, then run the rolling macro again.", _
               vbInformation, "SheetList created"
        ctlWs.Activate
        Set EnsureSheetListReady = Nothing
        Exit Function
    End If

    StandardizeSheetListHeaders ctlWs
    ctlWs.Activate
    Set EnsureSheetListReady = ctlWs
...
Private Function CreateSheetListFromVisibleSheets() As Worksheet
    ...
    On Error Resume Next
    Set out = ThisWorkbook.Worksheets("SheetList")
    On Error GoTo 0
    ...
    StandardizeSheetListHeaders out

    i = 2
    For Each ws In ThisWorkbook.Worksheets
        If ws.Visible = xlSheetVisible Then
            If StrComp(ws.Name, out.Name, vbTextCompare) <> 0 Then
                out.Cells(i, 1).Value = ws.Name
                i = i + 1
            End If
        End If
    Next ws

    Set CreateSheetListFromVisibleSheets = out

End Function

Private Sub StandardizeSheetListHeaders(ByVal ctlWs As Worksheet)

    Dim oldHeader As String
    oldHeader = Trim$(CStr(ctlWs.Range("A1").Value))

    If StrComp(oldHeader, "Visible Sheet Names", vbTextCompare) = 0 Then
        ctlWs.Range("A1").Value = "Target Sheet"
    ElseIf Len(oldHeader) = 0 Then
        ctlWs.Range("A1").Value = "Target Sheet"
    End If

    ctlWs.Range("B1").Value = "Source Column"
    ctlWs.Range("C1").Value = "Method"
    ctlWs.Range("D1").Value = "Freeze Max Level"
    ctlWs.Range("E1").Value = "Direction"
...
Private Function ConfirmSheetListReminder(ByVal ctlWs As Worksheet) As Boolean
    ...
    readiness = GetSheetListReadinessSummary(ctlWs)

    msg = "Control sheet found: [" & sheetLabel & "]." & vbCrLf & vbCrLf & _
          "Please confirm the meaning of each column before running:" & vbCrLf & vbCrLf & _
          "Column A = Target sheet name to process" & vbCrLf & _
          "Column B = Source month column to roll (number or letters, e.g. 3 or C)" & vbCrLf & _
          "           This should normally be filled for each sheet row you want to run." & vbCrLf & _
          "Column C = Method" & vbCrLf & _
          "           INSERT / I = insert next column and roll into it" & vbCrLf & _
          "           UNGROUP / U = only ungroup the target column; no extra insert" & vbCrLf & _
          "           If blank, default = INSERT" & vbCrLf & _
          "Column D = Freeze Max Level" & vbCrLf & _
          "           This means the highest formula level to preserve instead of rolling." & vbCrLf & _
          "           Formula hierarchy:" & vbCrLf & _
          "             Level 0 = normal internal formulas; these roll normally" & vbCrLf & _
          "             Level 1 = direct other-sheet formulas" & vbCrLf & _
          "             Level 2 = parents of Level 1 formulas" & vbCrLf & _
          "             Level 3 = formulas with hard-coded + / - adjustments" & vbCrLf & _
          "                       such as A1+100, +100, SUM(A1:G1,100), SUM(A1:G1)-0.2" & vbCrLf & _
          "           Freeze Max Level entry:" & vbCrLf & _
          "             0 = freeze none; everything rolls normally" & vbCrLf & _
          "             1 = freeze Level 1 only" & vbCrLf & _
          "             2 = freeze Levels 1 and 2" & vbCrLf & _
          "             3 = freeze Levels 1, 2 and 3" & vbCrLf & _
          "           If blank, default = 2" & vbCrLf & _
          "Column E = Direction" & vbCrLf & _
          "           blank = normal forward roll" & vbCrLf & _
          "           REVERSE = copy back to previous column" & vbCrLf & _
          "           If blank, default = forward" & vbCrLf & vbCrLf & _
          readiness & vbCrLf & vbCrLf & _
          "Press OK to confirm SheetList has been reviewed, or Cancel to stop."
...
Private Function GetSheetListReadinessSummary(ByVal ctlWs As Worksheet) As String

    Dim lastRow As Long
    Dim r As Long
    Dim runRowCount As Long
    Dim missingSource As String
    Dim missingTarget As String
    Dim sampleCap As Long

    sampleCap = 8
    lastRow = LastUsedRowAny(ctlWs)

    For r = 2 To lastRow
        If Len(Trim$(CStr(ctlWs.Cells(r, 1).Value))) > 0 Or _
           Len(Trim$(CStr(ctlWs.Cells(r, 2).Value))) > 0 Or _
           Len(Trim$(CStr(ctlWs.Cells(r, 3).Value))) > 0 Or _
           Len(Trim$(CStr(ctlWs.Cells(r, 4).Value))) > 0 Or _
           Len(Trim$(CStr(ctlWs.Cells(r, 5).Value))) > 0 Then

            If Len(Trim$(CStr(ctlWs.Cells(r, 1).Value))) = 0 Then
                If CountCsvItems(missingTarget) < sampleCap Then
                    AppendCsvItem missingTarget, CStr(r)
                End If
            Else
                runRowCount = runRowCount + 1
                If Len(Trim$(CStr(ctlWs.Cells(r, 2).Value))) = 0 Then
                    If CountCsvItems(missingSource) < sampleCap Then
                        AppendCsvItem missingSource, CStr(r)
                    End If
                End If
            End If
        End If
    ...
    GetSheetListReadinessSummary = "Detected " & runRowCount & " target row(s) listed in Column A."
    If Len(missingSource) > 0 Then
        GetSheetListReadinessSummary = GetSheetListReadinessSummary & vbCrLf & _
            "Please check Column B (Source Column). It is blank on row(s): " & missingSource
    Else
        GetSheetListReadinessSummary = GetSheetListReadinessSummary & vbCrLf & _
            "Column B appears filled for the listed target rows."
    End If

    If Len(missingTarget) > 0 Then
        GetSheetListReadinessSummary = GetSheetListReadinessSummary & vbCrLf & _
            "Please check Column A (Target Sheet). Some partially filled row(s) have blank sheet name: " & missingTarget
    End If

    GetSheetListReadinessSummary = GetSheetListReadinessSummary & vbCrLf & _
        "Columns C:E may be left blank if you want the defaults."

End Function

Private Sub AppendCsvItem(ByRef s As String, ByVal itemText As String)
    If Len(s) = 0 Then
        s = itemText
    Else
        s = s & ", " & itemText
...
Private Function CountCsvItems(ByVal s As String) As Long
    If Len(s) = 0 Then
        CountCsvItems = 0
    Else
        CountCsvItems = UBound(Split(s, ",")) + 1
    End If
End Function
