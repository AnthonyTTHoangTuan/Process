...
Private Const FORMAT_PREV_TO_CURR As String = "PREV_TO_CURR"
Private Const FORMAT_CURR_TO_PREV As String = "CURR_TO_PREV"
...
'   B: current column (to roll OR to ungroup)
'   C: roll direction
'        LEFT  = old current month should end up on the LEFT of new current month
'        RIGHT = old current month should end up on the RIGHT of new current month
'   D: format direction
'        blank     = PREV>>CURR
'        "reverse" = CURR>>PREV
'
' RULES:
' - If B blank AND C blank AND D blank => skip row
...
Private Sub Process_Instructions_From_SheetList()
    ...
    Dim sName As String, colRef As String
    Dim rollDir As String, fmtRaw As String, fmtDir As String
    ...
    For r = 2 To lastRow
        ...
        rollDir = UCase$(Trim$(CStr(wsCfg.Cells(r, "C").Value2)))
        fmtRaw = Trim$(CStr(wsCfg.Cells(r, "D").Value2))
        fmtDir = NormalizeFormatDirection(fmtRaw)

        If sName = "" And colRef = "" And rollDir = "" And fmtRaw = "" Then GoTo NextRow
        ...
        If colRef = "" And rollDir = "" Then GoTo NextRow

        If colRef <> "" And rollDir = "" Then
            colIdx = ColumnToNumber(colRef)
            UngroupColumnSafe wsTarget, colIdx
            GoTo NextRow
        End If

        If rollDir <> "LEFT" And rollDir <> "RIGHT" Then GoTo NextRow
        If colRef = "" Then colRef = CStr(ActiveCell.Column)

        RollPack_OneColumn_BySetup wsTarget, rollDir, fmtDir, colRef
...
Private Function NormalizeFormatDirection(ByVal rawText As String) As String
    Dim s As String
    s = UCase$(Trim$(rawText))

    Select Case s
        Case "", "PREV>>CURR", "PREV-CURR", "PREVIOUS-CURRENT", "PREVIOUS TO CURRENT"
            NormalizeFormatDirection = FORMAT_PREV_TO_CURR

        Case "REVERSE", "CURR>>PREV", "CURR-PREV", "CURRENT-PREVIOUS", "CURRENT TO PREVIOUS"
            NormalizeFormatDirection = FORMAT_CURR_TO_PREV
...
Private Function FormatDirectionLabel(ByVal fmtDir As String) As String
    If UCase$(fmtDir) = FORMAT_CURR_TO_PREV Then
        FormatDirectionLabel = "CURR>>PREV"
    Else
        FormatDirectionLabel = "PREV>>CURR"
    End If
End Function

Private Sub RollPack_OneColumn_BySetup(ByVal ws As Worksheet, _
                                       ByVal rollDirection As String, _
                                       ByVal formatDirection As String, _
                                       ByVal currentCol As Variant)

    Dim internalInsertDirection As String
    Dim rollDir As String, fmtDir As String

    rollDir = UCase$(Trim$(rollDirection))
    fmtDir = NormalizeFormatDirection(formatDirection)

    ' USER meaning of Col C:
    ' LEFT  = old current month should end up on LEFT of new current month
    ' RIGHT = old current month should end up on RIGHT of new current month
    '
    ' INTERNAL engine meaning:
    ' RIGHT = insert new column to the RIGHT of selected column
    ' LEFT  = insert new column AT selected column
    '
    ' Therefore mapping is reversed.
    Select Case rollDir
        Case "LEFT"
            internalInsertDirection = "RIGHT"
        Case "RIGHT"
            internalInsertDirection = "LEFT"
        Case Else
            Exit Sub
    End Select

    Application.StatusBar = "RollPack: " & ws.Name & _
                            " | Roll=" & rollDir & _
                            " | Format=" & FormatDirectionLabel(fmtDir)

    RollPack_OneColumn ws, internalInsertDirection, currentCol
End Sub
