...
' F0 : freeze subtotal cells whose refs are ALL already-frozen F1/F2 cells
'      (applied after F1/F2 pass; used for Cases 2-5, i.e. Blank/REST/WEST/NEST)
' ─────────────────────────────────────────────────────────────────────────────
...
Private Sub ProcessCase4_WEST( _
    ...
    ' Write all formulas to target using WEST-aware column shifting.
    ' Because colCount blank columns were inserted at srcFirstCol, every internal
    ' ref that originally pointed to a column >= srcFirstCol has shifted right by
    ' colCount on the sheet.  Refs to columns < srcFirstCol are unchanged.
    ' The target sits at srcFirstCol, so self-refs (same column as source) also
    ' need +colCount to keep pointing at the now-shifted source column.
    WriteArrayToRange_WEST arrVal, arrFmlA1, tgtRng, srcFirstCol, colCount, ws.Name

    ' Freeze F1+F2+F0 on (shifted) source
...
'================================================================================
' WriteArrayToRange_WEST
'   Like WriteArrayToRange_AllFormulas but uses WEST-aware column shifting.
'
'   When WEST inserts colCount blank columns at insertAtCol (= original srcFirstCol),
'   every cell on the sheet at column >= insertAtCol shifts right by colCount.
'   The new target sits at insertAtCol, holding copies of the original source
'   formulas.  Those formulas may reference:
'     (a) columns < insertAtCol  → unchanged (they did not shift)
'     (b) columns >= insertAtCol → shifted right by colCount on the sheet,
'         so the formula must also say colNum + colCount
'   External sheet refs are always left verbatim.
'================================================================================
Private Sub WriteArrayToRange_WEST( _
        ByVal arrVal      As Variant, _
        ByVal arrFmlA1    As Variant, _
        ByVal tgtRng      As Range, _
        ByVal insertAtCol As Long, _
    ...
    For r = 1 To tgtRng.Rows.Count
        For co = 1 To tgtRng.Columns.Count
            ...
            If Len(fA1) > 1 And Left$(fA1, 1) = "=" Then
                Dim fShifted As String
                fShifted = ShiftInternalColRefs_WEST(fA1, sheetName, insertAtCol, colCount)
                On Error Resume Next
                tgtRng.Cells(r, co).Formula = fShifted
                On Error GoTo 0
            Else
...
'================================================================================
' ShiftInternalColRefs_WEST
'   Rewrites an A1-notation formula for the WEST case.
'   For each bare (same-sheet) relative column reference:
'     • If original column number >= insertAtCol  → add colCount  (it shifted right)
'     • If original column number <  insertAtCol  → leave unchanged
'   Absolute column refs ($X) and external sheet refs are left verbatim.
'================================================================================
Private Function ShiftInternalColRefs_WEST( _
    ...
    Do While i <= n
        ...
        ' ── Quoted sheet name → external, copy verbatim ──────────────────────
        If ch = "'" Then
            Dim wqStart As Long : wqStart = i
            Dim wqEnd   As Long : wqEnd   = i + 1
            Do While wqEnd <= n
                If Mid$(formula, wqEnd, 1) = "'" Then
                    If wqEnd + 1 <= n And Mid$(formula, wqEnd + 1, 1) = "'" Then
                        wqEnd = wqEnd + 2
                    Else
                        Exit Do
                    End If
                ...
                End If
            Loop
            If wqEnd <= n And Mid$(formula, wqEnd, 1) = "'" Then
                If wqEnd + 1 <= n And Mid$(formula, wqEnd + 1, 1) = "!" Then
                    Dim wqTok As String
                    wqTok = Mid$(formula, wqStart, wqEnd - wqStart + 2)
                    out = out & wqTok
                    i = wqEnd + 2
        ...
        ' ── Unquoted identifier followed by ! → external, copy verbatim ──────
        If IsColStartChar(ch) Then
            Dim widStart As Long : widStart = i
            Dim wj       As Long : wj = i + 1
            Do While wj <= n And IsSheetIdentChar(Mid$(formula, wj, 1))
                wj = wj + 1
            Loop
            If wj <= n And Mid$(formula, wj, 1) = "!" Then
                Dim widTok As String : widTok = Mid$(formula, widStart, wj - widStart + 1)
                out = out & widTok
                i = wj + 1
                i = CopyAddressVerbatim(formula, i, out)
                GoTo WContinue
        ...
        If IsColStartChar(ch) Or ch = "$" Then
            Dim wAddrStart As Long  : wAddrStart = i
            Dim wColAbs    As Boolean : wColAbs = False
            Dim wColLet    As String  : wColLet = ""
            Dim wRowAbs    As Boolean : wRowAbs = False
            Dim wRowDig    As String  : wRowDig = ""
            Dim wk         As Long    : wk = i

            If Mid$(formula, wk, 1) = "$" Then wColAbs = True : wk = wk + 1
            Do While wk <= n And IsLetterAZ(Mid$(formula, wk, 1))
                wColLet = wColLet & Mid$(formula, wk, 1) : wk = wk + 1
            Loop
            If Len(wColLet) > 0 And Len(wColLet) <= 3 Then
                If wk <= n And Mid$(formula, wk, 1) = "$" Then wRowAbs = True : wk = wk + 1
                Do While wk <= n And IsDigit09(Mid$(formula, wk, 1))
                    wRowDig = wRowDig & Mid$(formula, wk, 1) : wk = wk + 1
                Loop
                If Len(wRowDig) > 0 Then
                    Dim wPrevCh As String : wPrevCh = ""
                    If wAddrStart > 1 Then wPrevCh = Mid$(formula, wAddrStart - 1, 1)
                    If wPrevCh <> "!" Then
                        ' Internal ref – conditionally shift relative column
                        If Not wColAbs Then
                            Dim wColNum As Long
                            wColNum = ColLettersToNum(UCase$(wColLet))
                            If wColNum >= insertAtCol Then
                                wColNum = wColNum + colCount
                                If wColNum >= 1 And wColNum <= 16384 Then
                                    wColLet = NumToColLetters(wColNum)
                                End If
                            End If
                            ' else: col < insertAtCol → leave unchanged
                        End If
                        Dim wTok As String
                        wTok = IIf(wColAbs, "$", "") & wColLet
                        wTok = wTok & IIf(wRowAbs, "$", "") & wRowDig
                        out = out & wTok
                        i = wk
                        GoTo WContinue
                    End If
...
'   No further propagation beyond one hop for F1/F2.
'
' ── F0 SUBTOTAL RULE (Step 3) ────────────────────────────────────────────────
'   After F1 and F2 cells are frozen to values, a further pass identifies
'   "subtotal" cells whose formulas reference ONLY source-range cells that were
'   already frozen in the F1/F2 pass.  These cells are frozen to values as F0.
'
'   Example:
'     N10, N11, N12  → frozen as F1 or F2 (they pull from external data)
'     N13 = SUM(N10,N11,N12)  → all refs are now values → N13 becomes F0 (frozen)
'
'   The pass repeats until no new F0 cells are found (handles chained subtotals).
'   A cell with NO internal source-range references is never treated as F0.
'================================================================================
Private Sub FreezeRange_DirectExtOnly( _
    ...
    '   (they will be reconsidered in Step 3 as F0 candidates).
    Dim fc As Range
    On Error Resume Next
    ...
    ' Track which source-range addresses were frozen in this pass (F1 or F2).
    Dim frozenSet As Object
    Set frozenSet = CreateObject("Scripting.Dictionary")
    frozenSet.CompareMode = vbTextCompare
    ...
    For Each c In fc.Cells
        ...
        If IsDirectExternalFormula(fml, ws.Name) Then
            ...
            c.Value2 = v1
            frozenSet(UCase$(c.Address(False, False))) = True
            GoTo NextCell
        End If
        ...
        '       and reconsider in Step 3 (F0 subtotal pass).

NextCell:
    Next c

    ' ── Step 3: F0 subtotal freeze ────────────────────────────────────────────
    ' A source cell is F0 if it is still a formula (not yet frozen) and ALL of
    ' its same-sheet cell references resolve exclusively to cells that were frozen
    ' in Step 2 (i.e. they are now plain values in frozenSet).
    ' Typical pattern: a subtotal row such as =SUM(N10,N11,N12) where N10:N12
    ' were all frozen to values as F1/F2 above.
    ' We loop until no further F0 cells are found (handles chained subtotals).
    If frozenSet.Count = 0 Then Exit Sub   ' nothing was frozen → no F0 candidates

    Dim anyFrozen As Boolean
    Do
        anyFrozen = False

        ' Re-scan: only formula cells remaining (non-frozen ones)
        Dim fc0 As Range
        On Error Resume Next
        Set fc0 = rng.SpecialCells(xlCellTypeFormulas)
        On Error GoTo 0
        If fc0 Is Nothing Then Exit Do

        Dim c0 As Range
        For Each c0 In fc0.Cells
            Dim addr0 As String
            addr0 = UCase$(c0.Address(False, False))
            If frozenSet.Exists(addr0) Then GoTo NextF0Cell   ' already frozen

            Dim fml0 As String : fml0 = c0.Formula

            ' Skip if the cell has any direct-external refs (already handled as F1)
            If IsDirectExternalFormula(fml0, ws.Name) Then GoTo NextF0Cell

            ' Skip if the cell references any whole-sheet F1 cell (already F2)
            If FormulaReferencesAnyInSet(fml0, ws.Name, f1Set) Then GoTo NextF0Cell

            ' Check: does every internal cell reference in this formula land in frozenSet?
            ' If yes → this is an F0 subtotal cell → freeze it.
            If FormulaReferencesOnlyFrozen(fml0, ws.Name, rng, frozenSet) Then
                Dim v0 As Variant : v0 = c0.Value2
                c0.Value2 = v0
                frozenSet(addr0) = True
                anyFrozen = True
            End If

NextF0Cell:
        Next c0
    Loop While anyFrozen   ' iterate until no new F0 cells found

End Sub


'================================================================================
' FormulaReferencesOnlyFrozen
'   Returns True if the formula qualifies as F0 (subtotal of frozen cells).
'   Rules:
'     • Only same-sheet bare cell references that fall INSIDE the source range
'       are examined ("in-range refs").
'     • References to cells OUTSIDE the source range (e.g. a neighbouring column
'       like M176 when source is column L) are ignored – they do not block F0.
'     • External sheet refs (Sheet!Addr) are always ignored.
'     • Returns True when there is at least one in-range ref AND every in-range
'       ref is already present in frozenSet (frozen as F1 or F2).
'     • Returns False if there are NO in-range refs at all (conservative – we
'       do not freeze plain constants or purely-external-formula cells via F0).
'================================================================================
Private Function FormulaReferencesOnlyFrozen( _
        ByVal formula   As String, _
        ByVal sheetName As String, _
        ByVal srcRng    As Range, _
        ByVal frozenSet As Object) As Boolean

    Dim n  As Long : n = Len(formula)
    Dim i  As Long : i = 1
    Dim ch As String
    Dim internalRefCount As Long : internalRefCount = 0
    Dim allFrozen        As Boolean : allFrozen = True
    ...
    Do While i <= n
        ...
        If ch = """" Then
            i = i + 1
            Do While i <= n
                Dim qchF As String : qchF = Mid$(formula, i, 1)
                i = i + 1
                If qchF = """" Then
                    If i <= n And Mid$(formula, i, 1) = """" Then
                        i = i + 1
        ...
        ' Skip quoted sheet name + address (external)
        If ch = "'" Then
            i = i + 1
            Do While i <= n
                Dim qc2F As String : qc2F = Mid$(formula, i, 1)
                i = i + 1
                If qc2F = "'" Then
                    If i <= n And Mid$(formula, i, 1) = "'" Then
                        i = i + 1
        ...
        If IsLetterAZ(ch) Then
            Dim idSF As Long : idSF = i
            Dim jF   As Long : jF = i + 1
            Do While jF <= n And IsSheetIdentChar(Mid$(formula, jF, 1))
                jF = jF + 1
            Loop
            If jF <= n And Mid$(formula, jF, 1) = "!" Then
                ' External: skip qualifier + ! + address
                i = jF + 1
                i = SkipCellAddr(formula, i)
                GoTo NextChF
            End If
            i = idSF   ' not external – fall through to bare address detection
        End If

        ' Try to parse bare cell address (internal ref)
        If IsLetterAZ(ch) Or ch = "$" Then
            Dim colAbsF  As Boolean : colAbsF = False
            Dim colLF    As String  : colLF = ""
            Dim rowAbsF  As Boolean : rowAbsF = False
            Dim rowDF    As String  : rowDF = ""
            Dim kF       As Long    : kF = i

            If Mid$(formula, kF, 1) = "$" Then colAbsF = True : kF = kF + 1
            Do While kF <= n And IsLetterAZ(Mid$(formula, kF, 1))
                colLF = colLF & Mid$(formula, kF, 1) : kF = kF + 1
            Loop
            If Len(colLF) >= 1 And Len(colLF) <= 3 Then
                If kF <= n And Mid$(formula, kF, 1) = "$" Then
                    rowAbsF = True : kF = kF + 1
                End If
                Do While kF <= n And IsDigit09(Mid$(formula, kF, 1))
                    rowDF = rowDF & Mid$(formula, kF, 1) : kF = kF + 1
                Loop
                If Len(rowDF) > 0 Then
                    Dim bareF As String
                    bareF = UCase$(colLF) & rowDF
                    Dim refRow As Long : refRow = CLng(rowDF)
                    Dim refCol As Long : refCol = ColLettersToNum(UCase$(colLF))
                    Dim inSrcRng As Boolean
                    inSrcRng = (refRow >= srcRng.Row And _
                                refRow <= srcRng.Row + srcRng.Rows.Count - 1 And _
                                refCol >= srcRng.Column And _
                                refCol <= srcRng.Column + srcRng.Columns.Count - 1)

                    If inSrcRng Then
                        ' Reference is inside the source range – must be frozen for F0
                        internalRefCount = internalRefCount + 1
                        If Not frozenSet.Exists(bareF) Then
                            allFrozen = False
                        End If
                    End If
                    ' References outside the source range (e.g. M176 when source is col L)
                    ' are simply ignored – they do not block F0 qualification.
                    i = kF
                    GoTo NextChF
                End If
            End If
...
NextChF:
    ...
    ' Return True only if there was at least one internal source ref and all were frozen
    FormulaReferencesOnlyFrozen = (internalRefCount > 0) And allFrozen
