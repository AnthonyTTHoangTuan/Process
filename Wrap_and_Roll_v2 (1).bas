...
'========================
' SheetList (start row 2):
'   A: sheet name (blank => ActiveSheet)
'   B: column (to roll OR to ungroup)
'   C: direction (LEFT / RIGHT) — ignored when D overrides
'   D: mode (optional)
'       "Arabica" => layout is Current-Left / Previous-Right; forces a LEFT roll
'       "Ungroup" => no insert; copy col B onto col B+1, then ungroup col B+1
'
' RULES:
' - B blank AND C blank AND D blank => skip row
' - D = "Arabica" => LEFT roll on col B (C value ignored)
' - D = "Ungroup" => copy col B -> col B+1, ungroup col B+1 (no roll logic)
' - B not blank AND C blank AND D blank => ungroup column B (original behaviour)
'========================
Private Sub Process_Instructions_From_SheetList()
    Dim wsCfg As Worksheet, lastRow As Long, r As Long
    Dim sName As String, colRef As String, dir As String, modeD As String
    ...
    For r = 2 To lastRow
        ...
        modeD  = UCase$(Trim$(CStr(wsCfg.Cells(r, "D").Value2)))

        ' Skip fully blank rows
        If sName = "" And colRef = "" And dir = "" And modeD = "" Then GoTo NextRow
        ...
        ' ── Column D = "ARABICA": Current on Left / Previous on Right ───────
        '    Overrides C; always performs a LEFT roll on col B
        If modeD = "ARABICA" Then
            If colRef = "" Then colRef = CStr(ActiveCell.Column)
            RollPack_OneColumn wsTarget, "LEFT", colRef
            GoTo NextRow
        End If

        ' ── Column D = "UNGROUP": copy col B -> col B+1, ungroup col B+1 ────
        If modeD = "UNGROUP" Then
            If colRef = "" Then GoTo NextRow          ' need a source column
            colIdx = ColumnToNumber(colRef)
            RollCopy_Ungroup wsTarget, colIdx
            GoTo NextRow
        End If

        ' ── Original behaviour (no D value) ─────────────────────────────────
        If colRef = "" And dir = "" Then GoTo NextRow

        ' B filled, C blank => plain ungroup (legacy rule)
...
'========================
' UNGROUP ROLL
' Copies source column onto srcColIdx+1 (no insert), then ungroups that column.
' Formula behaviour mirrors a RIGHT roll: all formulas (incl. external) are kept.
'========================
Private Sub RollCopy_Ungroup(ByVal ws As Worksheet, ByVal srcColIdx As Long)
    Dim dstColIdx As Long: dstColIdx = srcColIdx + 1
    Dim lastRow As Long:   lastRow   = GetLastUsedRow(ws)
    If lastRow < 1 Then Exit Sub

    Dim lastCol As Long: lastCol = GetLastUsedCol(ws)
    If dstColIdx > lastCol Then
        ' Destination column is beyond used range; still safe to write into it
    End If

    ' 1. Copy column formats (number format, fill, font, borders, width)
    CopyColumnFormats ws, srcColIdx, dstColIdx

    ' 2. Copy content: keep all formulas including external refs (same as RIGHT roll rule)
    BuildNewColumnFromOld_FAST ws, srcColIdx, dstColIdx, lastRow, ws.Name, _
                               freezeIfSameRowRightRef:=False, _
                               includeExternalFormulasOnNewCol:=True

    ' 3. Ungroup the destination column (expose it if it was grouped/hidden)
    UngroupColumnSafe ws, dstColIdx
End Sub
...
Private Sub RollPack_OneColumn(ByVal ws As Worksheet, ByVal direction As String, ByVal targetCol As Variant)
    ...
    ElseIf direction = "LEFT" Then
        ...
        ' Use A1-based copy for LEFT roll.
        ' Reason: R1C1 copy shifts all absolute-target offsets by -1 because the source
        ' cell moved from col rollColIdx to col rollColIdx+1 during the insert.
        ' e.g. SUM(N130:Q130) in old Q -> R1C1 in R has C[-4] (N) & C[0] (R).
        ' Writing that R1C1 to the new Q gives C[-4]=M (wrong!) & C[0]=Q.
        ' Reading A1 instead gives SUM(N130:R130); we then fix R->Q to get SUM(N130:Q130).
        BuildNewColumnFromOld_LEFT_A1 ws, oldColIdx, newColIdx, lastRow, ws.Name
        ...
        ' NOTE: FixRefs_TwoLeftToOneLeft is intentionally NOT called here.
        ' With A1 copy + old->new col letter fix, column references land correctly
        ' without offset compensation. Calling it would incorrectly shift col-2 refs.
    End If
End Sub
...
' - LEFT: external refs => value; internal => maybe formula
        ...
        If VarType(arrF(i, 1)) = vbString Then
            ...
            If Len(f) > 0 And Left$(f, 1) = "=" Then
                If includeExternalFormulasOnNewCol Then
                    ' RIGHT / Ungroup: keep formula always
                    arrOut(i, 1) = f
                Else
                    ' LEFT / Arabica: external => value; internal => maybe formula
...
'========================================================
' LEFT ROLL: build destination column using A1 formulas (array-based)
'
' WHY A1 and not R1C1:
'   When a column is inserted at rollColIdx, the old cell shifts from
'   col rollColIdx to col rollColIdx+1. Excel auto-adjusts the A1 formula
'   so absolute column targets stay correct (N130 stays N130), but R1C1
'   relative offsets shift by -1 (C[-3] becomes C[-4]).
'   Copying R1C1 verbatim to the new col misapplies offsets one step left:
'     SUM(N130:Q130) -> stored as C[-4]:C[0] in old col (now at +1)
'     -> written to new col gives C[-4]=M, so SUM(M130:Q130). BUG.
'
'   Reading A1 instead preserves absolute column letters.
'   We then replace references to the OLD column letter (e.g. "R") with
'   the NEW column letter (e.g. "Q") to fix range-ends that followed
'   the displaced cell:  SUM(N130:R130) -> SUM(N130:Q130).
'
' Filtering (same semantics as LEFT path in BuildNewColumnFromOld_FAST):
'   External formula        => freeze to value
'   Same-row RIGHT ref      => freeze to value
'   Everything else         => copy with old-col->new-col letter fix
'========================================================
Private Sub BuildNewColumnFromOld_LEFT_A1(ByVal ws As Worksheet, ByVal oldColIdx As Long, ByVal newColIdx As Long, _
                                          ByVal lastRow As Long, ByVal thisSheetName As String)

    Dim rngOld As Range, rngNew As Range
    Dim arrVal As Variant, arrA1 As Variant, arrRC As Variant, arrOut As Variant
    Dim i As Long, f As String, fRC As String
    Dim oldLetter As String, newLetter As String

    oldLetter = NumberToColumn(oldColIdx)   ' displaced col, e.g. "R"
    newLetter = NumberToColumn(newColIdx)   ' inserted col,  e.g. "Q"

    Set rngOld = ws.Range(ws.Cells(1, oldColIdx), ws.Cells(lastRow, oldColIdx))
    Set rngNew = ws.Range(ws.Cells(1, newColIdx), ws.Cells(lastRow, newColIdx))

    arrVal = rngOld.Value2       ' fallback: paste value
    arrA1  = rngOld.Formula      ' A1  — absolute col letters preserved across cell move
    arrRC  = rngOld.FormulaR1C1  ' R1C1 — used only by detection helpers
    arrOut = arrVal              ' default: value

    For i = 1 To UBound(arrA1, 1)
        If VarType(arrA1(i, 1)) = vbString Then
            f   = CStr(arrA1(i, 1))
            fRC = CStr(arrRC(i, 1))
            If Len(f) > 0 And Left$(f, 1) = "=" Then
                ' Rule 1: external formula => keep value (not in new current-month col)
                If IsExternalFormulaFast_R1C1(fRC, thisSheetName) Then GoTo NextCell
                ' Rule 2: same-row ref pointing RIGHT => keep value (forward-ref freeze)
                If HasSameRowRightRefR1C1_FAST(fRC, i, oldColIdx) Then GoTo NextCell
                ' Rule 3: valid internal formula
                '   Fix displaced old-col letter refs that followed the insert:
                '   e.g. SUM(N130:R130) where R = old rolled col -> SUM(N130:Q130)
                If InStr(1, f, oldLetter, vbTextCompare) > 0 Then
                    f = ReplaceColRefs_SkipStrings_FAST(f, oldLetter, newLetter)
                End If
                arrOut(i, 1) = f
            End If
        End If
NextCell:
    ...
    rngNew.Formula = arrOut   ' write as A1
End Sub
...
Private Sub CopyLegacyNoteWithFormatting(ByVal srcComment As Comment, ByVal dstCell As Range)
    ...
    ' copy rich runs (bold/italic/underline) - per character (comment objects only, not per cell)
