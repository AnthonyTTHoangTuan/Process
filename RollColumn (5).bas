...
Sub RollColumns()
    ...
    For i = 2 To lastRow
        ...
        Dim isUngrouped As Boolean
        isReverse   = (UCase(layout) = "REVERSE" Or UCase(layout) = "PREVRIGHT")
        isUngrouped = (UCase(layout) = "UNGROUPED")
        ...
        ' Ungrouped: ungroup outer neighbour of the new column
        If isUngrouped Then
            Dim outerCol As Long
            If UCase(insertDir) = "LEFT" Then
                outerCol = newCol - 1   ' left of new col
            Else
                outerCol = newCol + 1   ' right of new col
            End If
            If outerCol >= 1 And outerCol <= wsTarget.Columns.Count Then
                On Error Resume Next
                wsTarget.Columns(outerCol).Ungroup
                On Error GoTo 0
            End If
        End If
...
Private Sub ProcessRoll(ws As Worksheet, _
    ...
    ' --- COMMENTS ---
    ' Rules:
    '   New Current col : always blank — no comments at all
    '   Previous col    : clear everything, then inherit from existingCol
    '                     (existingCol = the column that WAS current before insert)
    '
    ' Two cases depending on which column is new vs existing:
    '
    ' Case A: newIsCurrent = True  (newCol=Current, existingCol=Previous)
    '   existingCol held the current-period comments before insert.
    '   They must move to previousCol (=existingCol itself — already there),
    '   but the rule says Previous clears first then copies from "was Current".
    '   Since existingCol IS previousCol, we just leave its comments in place
    '   (clear+copy from self = no net change).  Then clear newCol (Current).
    '
    ' Case B: newIsCurrent = False (newCol=Previous, existingCol=Current)
    '   existingCol stays as Current — its comments are KEPT on currentCol.
    '   newCol becomes Previous — clear it (it's blank anyway), then copy
    '   comments from existingCol (the current-period source) into newCol.
    '   Finally clear currentCol (existingCol) comments per "Current = blank".

    If newIsCurrent Then
        ' existingCol becomes Previous: comments already there, nothing to move.
        ' Just ensure newCol (Current) is blank.
        Call ClearColumnComments(ws, newCol, lastRow)
    Else
        ' newCol becomes Previous:
        '   1. Copy existingCol comments → newCol  (Previous inherits from Current)
        '   2. Clear existingCol (currentCol) comments  (Current = blank)
        Call ClearColumnComments(ws, newCol, lastRow)          ' newCol is fresh insert, but be safe
        Call CopyColumnComments(ws, existingCol, newCol, lastRow)
        Call ClearColumnComments(ws, existingCol, lastRow)     ' existingCol = currentCol, must be blank
    End If
...
Private Sub ProcessFormulas(ws As Worksheet, _
    ...
    For r = 1 To lastRow
        ...
        ' --- Copy formula/value to new column ---
        ' Assigning .Formula to an offset cell makes Excel auto-adjust relative refs
