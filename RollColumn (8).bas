...
Private Sub ProcessRoll(ws As Worksheet, _
    ...
    ' --- COMMENTS ---
    ' Rule: Previous col = always has comments (inherited from pre-roll Current)
    '       Current col  = always blank
    '
    ' existingCol was Current before the roll, so it holds the source comments.
    '
    ' Case A: newIsCurrent = True  (newCol=Current, existingCol=Previous)
    '   newCol (Current)      => clear (fresh insert, but be safe)
    '   existingCol (Previous)=> copy comments from existingCol to itself = keep as-is
    '   Net: just clear newCol.
    '
    ' Case B: newIsCurrent = False (newCol=Previous, existingCol=Current)
    '   newCol (Previous)     => clear first, then copy from existingCol
    '   existingCol (Current) => clear AFTER copy so no comments are lost
    '   Net: copy existingCol->newCol, then clear existingCol.

    If newIsCurrent Then
        ' Current = newCol: blank. Previous = existingCol: comments stay.
        Call ClearColumnComments(ws, newCol, lastRow)
    Else
        ' Previous = newCol: inherits from existingCol (pre-roll Current).
        ' Current  = existingCol: cleared after copy.
        Call ClearColumnComments(ws, newCol, lastRow)
