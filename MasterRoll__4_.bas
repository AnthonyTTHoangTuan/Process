...
'   Returns True if EVERY internal (same-sheet) bare cell reference in the
'   formula resolves to an address already present in frozenSet.
'   External sheet refs are ignored (they don't count as internal refs).
'   If the formula has NO internal refs at all, returns False (conservative –
'   we don't freeze constants or purely-external formulas via F0).
'================================================================================
        ...
        If IsLetterAZ(ch) Or ch = "$" Then
            ...
            If Len(colLF) >= 1 And Len(colLF) <= 3 Then
                ...
                If Len(rowDF) > 0 Then
                    ...
                    ' Only count if the referenced cell is within the source range
                    ' (we don't penalise references to cells outside the roll range)
                    Dim refRow As Long : refRow = CLng(rowDF)
                    Dim refCol As Long : refCol = ColLettersToNum(UCase$(colLF))
                    If refRow >= srcRng.Row And _
                       refRow <= srcRng.Row + srcRng.Rows.Count - 1 And _
                       refCol >= srcRng.Column And _
                       refCol <= srcRng.Column + srcRng.Columns.Count - 1 Then
