...
'   Freezes cells on the source range whose formula DIRECTLY references
'   another sheet or workbook (F1 cells only).
'
'   Cells that only reference same-sheet cells are left as formulas,
'   regardless of whether those same-sheet cells themselves depend on
'   external data.
'
'   Rule confirmed by user examples:
'     Q12  = ROUND(SUMIFS('Aggregate TB'!...),2)  → direct external → FREEZE ✓
'     O12  = ROUND(Q12-SUM(G12:OFFSET(O12,0,-1))) → same-sheet refs only → KEEP ✓
'     N21  = ROUND(SUM(N10,N11,...),2)             → same-sheet refs only → KEEP ✓
'     SUM cell = ROUND(SUM(O10,O11,...),2)         → same-sheet refs only → KEEP ✓
'
'   The previously attempted F2 BFS propagation was too aggressive and
'   incorrectly froze aggregation formulas. Removed entirely.
'================================================================================
Private Sub FreezeRange_DirectExtOnly( _
    ...
    Dim c As Range
    For Each c In fc.Cells
        ' Only freeze if the formula's own text directly references another sheet
        If IsDirectExternalFormula(c.Formula, ws.Name) Then
            Dim v As Variant : v = c.Value2
            c.Value2 = v
        End If
        ' Same-sheet formulas (including those that reference F1 cells) stay as formulas
    Next c
