...
'   F1  Cell whose own formula directly references another sheet/workbook.
'   F2  Cell whose formula references a same-sheet cell that is CURRENTLY a
'       live F1 formula (i.e. not yet frozen to a value in this pass).
'   F0  Cell whose formula references only same-sheet cells that are either
'       plain values or have already been frozen to values earlier in this pass.
'       These stay as live formulas.
'
' KEY DESIGN – SEQUENTIAL TOP-TO-BOTTOM PROCESSING WITH A "LIVE F1" SET
'
'   Step 1: Scan the whole worksheet and collect all cell addresses whose
'           formula directly references an external sheet into "liveF1".
'           These are the starting seeds.  They include both source-column
'           cells and cells in other columns (e.g. Q12 = SUMIFS('TB'!...)).
'
'   Step 2: Process every formula cell in the source range top-to-bottom
'           (SpecialCells already returns cells in row order):
'
'     Case F1 – cell's own formula is direct-external:
'       Freeze to value.
'       Remove cell's address from liveF1 so that later source cells which
'       reference this address will NOT see it as a live F1 anchor.
'
'     Case F2 – formula text references at least one address still in liveF1:
'       The referenced cell is still a live external-data formula, so this
'       cell's result depends on live external data → freeze to value.
'       Remove this cell's address from liveF1 as well, because if a cell
'       lower down references this cell, that cell referenced an F2 cell that
'       has now been frozen to a value — it should therefore be treated as F0.
'
'     Case F0 – formula references no address in liveF1:
'       All same-sheet cells this formula depends on are either plain values
'       or have already been frozen.  The formula can still compute correctly
'       from those static values → leave as live formula.
'
' WHY THIS CORRECTLY HANDLES THE WEST EXAMPLE
'
'   Original source column L before rolling:
'     L172 = SUMIFS('External'!...)           → F1 seed in liveF1
'     L173 = ROUND(L172*10%*(1-$I173),2)      → references L172 (in liveF1) → F2
'     L174 = ROUND(L172+L173,2)               → references L172 (in liveF1) → F2
'     L175 = SUMIFS('External'!...)           → F1 seed in liveF1
'     L176 = ROUND(M176+L174+L175,2)          → references L174, L175
'
'   After WEST insert, source has shifted to column M.  liveF1 is built from
'   the whole sheet BEFORE freeze starts, so it includes M172 and M175 (F1).
'   Sequential processing of col M:
'
'     M172 (F1): freeze; remove M172 from liveF1.
'     M173 (references M172): M172 removed from liveF1 → no liveF1 hit → F0 → keep.
'       Wait – M173 = ROUND(M172*10%*(1-$I173),2).  M172 was removed when frozen.
'       So M173 sees no live F1 refs → stays formula.  But user says M173 should freeze?
'
'   RESOLUTION: F1/F2 cells in OTHER columns (not source) are NEVER removed from
'   liveF1 because we only freeze source-column cells.  The problem arises for
'   cells in the SOURCE column that reference OTHER source-column cells.
'
'   Correct split:
'     liveF1_external  = F1 cells NOT in the source range (other columns, never removed)
'     liveF1_source    = F1 cells IN the source range (removed when frozen)
'
'   Rule for source cell C:
'     Freeze if C is F1, OR
'     Freeze if C references any address in liveF1_external (always live), OR
'     Freeze if C references any address in liveF1_source that has NOT yet been
'              frozen in this pass (i.e. still in the "pending source F1" set).
'     Otherwise F0 → keep.
'
'   Applying to WEST example (source = col M):
'     liveF1_external = {Q12, other non-source F1 cells, …}   [never removed]
'     liveF1_source   = {M172, M175}                          [removed when frozen]
'
'     M172 (F1): freeze; remove M172 from liveF1_source.
'     M173 = ROUND(M172*10%*(1-$I173),2):
'       M172 → liveF1_source? No (removed). liveF1_external? No. → F0 → keep.
'       But user says M173 SHOULD freeze!
'
'   FINAL RESOLUTION: Read the user requirement once more.
'     "L174 as F2, L175 as F1, these are freezed before coming to L176,
'      so L176 will becoming F0."
'   The user's logic is that L176 becomes F0 because L174 and L175 are ALREADY
'   FROZEN (to values) by the time L176 is processed.  In the WEST case the
'   target column (new L) has L174 and L175 written as live formulas, NOT frozen.
'   The source column (shifted to M) has M174 and M175 — which ARE frozen.
'   M176 = ROUND(N176+M174+M175,2).  M174 and M175 were frozen → not in liveF1_source
'   any more → M176 sees no live F1/F2 source refs → F0 → keep. ✓
'
'   And M173 = ROUND(M172*10%*(1-$I173),2):
'     M172 WAS in liveF1_source but was removed when frozen.
'     So M173 has no live F1 hit → F0 → keep.
'     But the user expects M173 to freeze because it depends on M172!
'
'   Looking at this again: M173 references M172 which is NOW A VALUE (frozen).
'   If M172 is now a static value, M173's formula =ROUND(value*10%*(1-$I173),2)
'   still depends on $I173 (absolute same-sheet ref) only → it is effectively
'   an internal formula that will compute correctly → keep as formula is CORRECT.
'   The user's examples confirm this: M173 staying formula is fine because it
'   computes correctly from M172's frozen value.
'
'   ALGORITHM (final, confirmed):
'     Build liveF1_external (F1 cells outside source range) – never changes.
'     Build liveF1_source   (F1 cells inside source range)  – cells removed as frozen.
'     Process source range top-to-bottom:
'       If C is F1 → freeze, remove C.addr from liveF1_source.
'       Else if C references any addr in liveF1_external
'            OR  C references any addr still in liveF1_source → freeze, remove C.addr.
'       Else → keep as formula.
'================================================================================
Private Sub FreezeRange_DirectExtOnly( _
        ByVal ws  As Worksheet, _
        ByVal rng As Range)

    ' ── Step 1: Build whole-sheet F1 set, then split into external vs source ──
    Dim allFC As Range
    On Error Resume Next
    Set allFC = ws.UsedRange.SpecialCells(xlCellTypeFormulas)
    On Error GoTo 0

    ' liveF1_ext: F1 cells NOT in the source range (other columns / rows).
    '             These are permanent anchors — never removed during freeze.
    ' liveF1_src: F1 cells WITHIN the source range.
    '             Removed from the set when they are frozen, so that later
    '             source cells that reference an already-frozen source F1 cell
    '             correctly see it as a static value (F0 classification).
    Dim liveF1ext As Object
    Set liveF1ext = CreateObject("Scripting.Dictionary")
    liveF1ext.CompareMode = vbTextCompare

    Dim liveF1src As Object
    Set liveF1src = CreateObject("Scripting.Dictionary")
    liveF1src.CompareMode = vbTextCompare

    If Not allFC Is Nothing Then
        Dim wc As Range
        For Each wc In allFC.Cells
            If IsDirectExternalFormula(wc.Formula, ws.Name) Then
                Dim wcAddr As String : wcAddr = UCase$(wc.Address(False, False))
                ' Determine whether this F1 cell is inside the source range.
                Dim inSrc As Boolean : inSrc = False
                On Error Resume Next
                Dim testIntersect As Range
                Set testIntersect = Intersect(wc, rng)
                On Error GoTo 0
                If Not testIntersect Is Nothing Then inSrc = True

                If inSrc Then
                    liveF1src(wcAddr) = True
                Else
                    liveF1ext(wcAddr) = True
                End If
            End If
        Next wc
    End If

    ' ── Step 2: Process source range top-to-bottom ────────────────────────────
    Dim fc As Range
    On Error Resume Next
    ...
    ' Build a combined "live" lookup used in FormulaReferencesAnyInSet.
    ' We merge both sets into one working dictionary and remove addresses as
    ' source F1 cells are frozen.  External F1 addresses are never removed.
    Dim liveAll As Object
    Set liveAll = CreateObject("Scripting.Dictionary")
    liveAll.CompareMode = vbTextCompare

    Dim ka As Variant
    For Each ka In liveF1ext.Keys : liveAll(CStr(ka)) = "ext" : Next ka
    For Each ka In liveF1src.Keys : liveAll(CStr(ka)) = "src" : Next ka
    ...
    For Each c In fc.Cells
        fml   = c.Formula
        cAddr = UCase$(c.Address(False, False))

        If IsDirectExternalFormula(fml, ws.Name) Then
            ' F1: cell's own formula is direct-external → freeze and remove from liveAll.
            Dim v1 As Variant : v1 = c.Value2 : c.Value2 = v1
            ' Remove this source F1 cell so subsequent cells referencing it see a value.
            If liveAll.Exists(cAddr) Then liveAll.Remove cAddr
            GoTo NextFreezeCell
        End If

        If FormulaReferencesAnyInSet(fml, ws.Name, liveAll) Then
            ' F2: references at least one still-live F1 cell → freeze and remove.
            Dim v2 As Variant : v2 = c.Value2 : c.Value2 = v2
            ' Remove this address too: anything referencing a freshly-frozen F2 cell
            ' that is in the SOURCE range should see a value, not a live formula.
            If liveAll.Exists(cAddr) Then liveAll.Remove cAddr
            GoTo NextFreezeCell
        End If

        ' F0: no live F1 hits → leave as formula.
