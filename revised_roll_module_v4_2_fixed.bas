...
Public Sub RunSelfTests_RollParser()
    ...
    ' Split into two Array() calls to stay under VBA's 25 line-continuation limit.
    Dim tests1 As Variant
    tests1 = Array( _
    ...
    ' Combine tests1 + tests2 into a single tests array.
    Dim tests As Variant
    Dim tTotal As Long: tTotal = UBound(tests1) + UBound(tests2) + 2
    ReDim tests(0 To tTotal - 1)
    Dim ti As Long
    For ti = 0 To UBound(tests1): tests(ti) = tests1(ti): Next ti
    For ti = 0 To UBound(tests2): tests(UBound(tests1) + 1 + ti) = tests2(ti): Next ti
...
Public Sub Test_ShowWorkbookBinding()
    MsgBox "ActiveWorkbook: " & ActiveWorkbook.Name & vbCrLf & _
           "ActiveSheet workbook: " & ActiveSheet.Parent.Name & vbCrLf & _
           "ThisWorkbook (code workbook): " & ThisWorkbook.Name, vbInformation, "Workbook Binding Check"
End Sub

Public Sub Test_ProcessActiveControlFirstRow()
    Dim ctlWs As Worksheet
    Dim hostWb As Workbook
    Dim targetSheetName As String
    Dim colSpec As String
    Dim methodText As String
    Dim freezeMaxText As String
    Dim directionText As String

    Set ctlWs = ActiveSheet
    Set hostWb = ctlWs.Parent

    targetSheetName = Trim$(CStr(ctlWs.Cells(2, 1).Value2))
    colSpec = Trim$(CStr(ctlWs.Cells(2, 2).Value2))
    methodText = Trim$(CStr(ctlWs.Cells(2, 3).Value2))
    freezeMaxText = Trim$(CStr(ctlWs.Cells(2, 4).Value2))
    directionText = Trim$(CStr(ctlWs.Cells(2, 5).Value2))

    ProcessOneControlRow hostWb, targetSheetName, colSpec, methodText, freezeMaxText, directionText

    MsgBox "First control row processed successfully in workbook [" & hostWb.Name & "].", vbInformation
End Sub
...
Private Sub ProcessOneControlRow( _
    ...
    If hostWb Is Nothing Then
        Err.Raise vbObjectError + 7005, "ProcessOneControlRow", _
                  "Host workbook is Nothing."
    End If

    If Len(targetSheetName) = 0 Then
        Err.Raise vbObjectError + 7000, "ProcessOneControlRow", _
                  "Blank target sheet name in control sheet."
    End If

    If Len(colSpec) = 0 Then
        Err.Raise vbObjectError + 7003, "ProcessOneControlRow", _
                  "Blank column spec for sheet [" & targetSheetName & "]."
    End If

    On Error Resume Next
    Set ws = hostWb.Worksheets(targetSheetName)
    On Error GoTo 0

    If ws Is Nothing Then
        Err.Raise vbObjectError + 7001, "ProcessOneControlRow", _
                  "Worksheet not found in workbook [" & hostWb.Name & "]: [" & targetSheetName & "]"
    End If
