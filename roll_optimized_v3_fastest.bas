...
'
'   BuildFormulaLevelsForSheetFast walks every formula cell in the sheet's
'   UsedRange.  The old code called it once per control-row, so if the same
'   worksheet appeared three times in the SheetList (three columns to roll)
'   the expensive BFS/dependency scan ran three times on the same data.
'
...
' ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Private mLevelMapCache As Object   ' Scripting.Dictionary(ws.Name → levelMap)
...
CleanFail:
    ...
    ' ★ OPT 1 — release cache on error path too
    Set mLevelMapCache = Nothing
    Set mLastRowCache = Nothing
...
Private Sub RollOneSheetOneColumn( _
    ...
    ' ★ OPT 1 / V3 — build level map only when it can actually be used
        ...
        If Not srcFormulaCells Is Nothing Then
            If Not mLevelMapCache Is Nothing Then
                If Not mLevelMapCache.Exists(ws.Name) Then
                    Set mLevelMapCache(ws.Name) = BuildFormulaLevelsForSheetFast(ws)
                End If
...
' ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
' OPTIMISATION 4 — Reuse one Scripting.Dictionary across all
'                   ExtractExplicitRefsFast calls in the BFS scan
'
'   The old code called ExtractExplicitRefsFast(…) which created a brand-new
'   Scripting.Dictionary for every formula cell, used it, then discarded it.
'   For a sheet with 300 formula cells that is 300 COM object instantiations
'   plus 300 GC teardowns, all inside a tight loop.
'
'   The new approach allocates one reusable dictionary (refsDict) once in
'   BuildFormulaLevelsForSheetFast, passes it to ExtractExplicitRefsFast_InPlace,
'   and calls refsDict.RemoveAll between iterations.  The public wrapper
'   ExtractExplicitRefsFast is kept unchanged so any other callers are unaffected.
' ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Private Function BuildFormulaLevelsForSheetFast(ByVal ws As Worksheet) As Object
    ...
    Dim refs As Object      ' ★ OPT 4 — one reusable dict for all iterations
    Dim n As Long
    Dim i As Long
    Dim idx As Long
    ...
    ' ★ OPT 4 — allocate reusable refs dict once; clear it between cells
    ...
    For Each c In formulaCells.Cells
        ...
        refs.RemoveAll   ' ★ OPT 4 — reuse instead of recreate
        ExtractExplicitRefsFast_InPlace CStr(c.Formula), wsNameUpper, refs
