...
Private Sub ProcessRoll(ws As Worksheet, _
    ...
    ' --- COMMENTS ---
    ' Current col: clear all comments
    Call ClearColumnComments(ws, currentCol, lastRow)

    ' Previous col inherits comments from what was the Current col before rolling.
    ' The "was Current" col is existingCol when existingCol=previousCol (it stayed as prev)
    ' or existingCol when existingCol=currentCol (new col becomes prev, existingCol = current).
    ' So: if existingCol IS the currentCol, copy its comments to previousCol (newCol) BEFORE clearing.
    If existingCol = currentCol Then
        ' Comments on existingCol (now current) should move to previousCol (newCol)
        Call CopyColumnComments(ws, existingCol, previousCol, lastRow)
        ' ClearColumnComments already ran on currentCol above, so comments are gone
    End If
    ' If existingCol = previousCol, its comments stay as-is (already on previous col)
